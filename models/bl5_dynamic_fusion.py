...
class BL5RunOnlyDynamicFusion(nn.Module):
    ...
    def __init__(
        ...
        if self.fusion_type not in {"cross_attn_gate", "simple_concat"}:
            raise ValueError(
                "model.fusion_type must be 'cross_attn_gate' or 'simple_concat', "
                f"got {self.fusion_type!r}"
            )
        if self.use_pam_encoder and self.fusion_type != "simple_concat":
            raise ValueError("Route-B PAM encoder is currently supported only with simple_concat fusion")
        if model_cfg.get("use_run") is not True and not self.use_learnable_run:
        ...
        if self.rna_pooling not in {"mean", "cls"}:
            raise ValueError(f"model.rna_pooling must be 'mean' or 'cls', got {self.rna_pooling!r}")
    ...
    def _init_new_weights(self) -> None:
        modules: list[nn.Module] = [self.run_encoder, self.classifier]
