#!/usr/bin/env bash
# Launch BL5-v4 PAM or NoPAM holdout-AGG run on two GPUs.
#
# Usage:
#   bash run/run_bl5_v4_holdout_agg.sh pam
#   bash run/run_bl5_v4_holdout_agg.sh nopam
#
# Environment overrides:
#   PYTHON_BIN=/path/to/python
#   CUDA_VISIBLE_DEVICES=0,1
#   MASTER_PORT=29531
#   TMUX_NAME=bl5_holdout_agg
#   FOREGROUND=1
...
if [ "$MODE" = "pam" ]; then
  CONFIG="configs/bl5_v4_pam_holdout_agg.yaml"
  OUTPUT_DIR="results/bl5_v4_pam_holdout_agg"
  TMUX_NAME="${TMUX_NAME:-bl5_pam_holdout_agg}"
elif [ "$MODE" = "nopam" ]; then
  CONFIG="configs/bl5_v4_nopam_holdout_agg.yaml"
  OUTPUT_DIR="results/bl5_v4_nopam_holdout_agg"
  TMUX_NAME="${TMUX_NAME:-bl5_nopam_holdout_agg}"
else
...
echo "========================================"
echo " BL5 holdout-AGG 2-GPU launcher"
echo "========================================"
echo "[INFO] Mode:    $MODE"
