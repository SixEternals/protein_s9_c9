#!/usr/bin/env bash
# Launch BL6-1-PAM-Gated-Fusion formal run on two GPUs.
#
# Usage:
#   bash run/run_bl6_1_formal_2gpu.sh
#   bash run/run_bl6_1_formal_2gpu.sh configs/bl6_1_pam_gated_fusion.yaml
#   bash run/run_bl6_1_formal_2gpu.sh configs/bl6_1_pam_gated_fusion.yaml results/bl6_1_pam_gated_fusion_rerun
#
# Environment overrides:
#   PYTHON_BIN=/path/to/python
#   CUDA_VISIBLE_DEVICES=0,1
#   MASTER_PORT=29532
#   TMUX_NAME=bl6_1_formal
#   FOREGROUND=1
...
echo "========================================"
echo " BL6-1 formal 2-GPU launcher"
echo "========================================"
...
if tmux has-session -t "$TMUX_NAME" 2>/dev/null; then
  ...
  echo "        Or choose another name: TMUX_NAME=bl6_1_formal_2 bash $0 $CONFIG ${OUTPUT_DIR:-}" >&2
  exit 1
fi
