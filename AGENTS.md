...
> ⚠️ **文档时效性警告**：
> 1. **`reborn_doc/过期/`**：存放已被整合或部分过期的历史文档。**任何 AI 助手均不得引用该路径下的文件作为当前事实依据**。
> 2. **`doc/`**：存放**最老一批旧模型**的历史文档（DeepFocus/R9、ConMismatch9/C9、CMPD 等师兄早期工作，以及 `unpacked/` 下的旧代码笔记）。这些文档描述的是项目重生**之前**的模型体系，与当前 BL0-BL6 代码基、坐标约定、数据口径均可能存在差异。**AI 助手禁止直接参照 `doc/` 路径下的内容指导当前模型开发、训练或评估**。
>
> 最新项目状态始终以 **`reborn_doc/todo.md`** 为准，实验结论以 **`reborn_doc/38_BL5_v4_PAM_组件消融总报告.md`** 和 **`reborn_doc/39_BL5_v4_PAM_泛化证据总报告.md`** 为准。
